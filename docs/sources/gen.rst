...
| encoding        | Java ファイルのエンコーディングです。                               | UTF-8         |      |
+-----------------+---------------------------------------------------------------------+---------------+------+
| overwrite       | ``true`` の場合、Dao インタフェースの Java ファイルを上書きします。 | true          |      |
+-----------------+---------------------------------------------------------------------+---------------+------+
...
.. code-block:: groovy
  ...
  dependencies {
      domaGenRuntime 'org.seasar.doma:doma-gen:2.0-beta-5-SNAPSHOT'
...
修正したテンプレートファイルは、ファイル名を変更せずに ``templateFilePrimaryDir`` パラメータに指定するディレクトリに配置します。
